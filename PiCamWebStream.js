...
class PiCamWebStream {
    constructor(server, width, height, fps) {
        this.width = width;
        this.height = height;
        this.wss = new WebSocket.Server({ server });
        this.vid_stream = spawn('raspivid', ['-t', '0', '-w', width, '-h', height, '-fps', fps ? fps : 12, '-o', '-', '-pf', 'baseline']);
        this.readStream = undefined;
        this.initListeners();
        this.splitter = new Splitter(new Buffer([0,0,0,1]));
    }
    ...
    startFeed() {
        console.info('starting feed');
        //vid_stream.stdout.pipe(process.stdout);
        this.readStream = this.vid_stream.stdout.pipe(this.splitter);
